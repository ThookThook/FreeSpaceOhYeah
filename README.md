...
`encode` creates an MP4 video containing the source file, a HARE header with the original size and SHA-256, checksummed data chunks, and ~10% XOR repair chunks. `decode` reads one or more encoded MP4 files, reconstructs the chunk stream, uses the repair chunks when a group has one missing/corrupt data chunk, writes the requested output file, and verifies the SHA-256 before reporting success.
...
- **Output side effect:** Writes `archive.mp4.encode.checkpoint` next to the MP4. The checkpoint records the source path, source size, SHA-256, HARE parameters, ECC settings, output list, and completion status.
...
- Per-chunk checksums, one XOR repair symbol per 10 data chunks (~10% ECC), single-missing/corrupt-chunk recovery per ECC group, and file-level SHA-256 verification on decode.
...
The repository contains the runnable scaffold for v1 with HARE Channel A, chunk checksums, and a Tornado-class XOR repair path. True fountain/rateless symbol scheduling beyond the current one-repair-per-10-data-chunks layout, true byte-offset resume, and true multi-volume splitting are still implementation TODOs. When CMake finds NVCC/CUDA, the NVIDIA GPU Suite add-on builds a CUDA kernel for luma frame synthesis; otherwise it builds a driver-detecting stub so the main CLI still compiles without a GPU toolchain.

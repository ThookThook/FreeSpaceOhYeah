...
int encode(const Opt&o){
 ...
 uint64_t sz=fs::file_size(o.input); auto sha=file_sha(o.input); string ck=o.resume&&!o.checkpoint.empty()?o.checkpoint:o.output+".encode.checkpoint"; if(!ends_with(ck,".encode.checkpoint")) die("encode checkpoint path must end with .encode.checkpoint"); encode_checkpoint(ck,o.input,o.output,sz,sha,"started",0); vector<uint8_t> payload; string magic="FSOYHARE1"; payload.insert(payload.end(),magic.begin(),magic.end()); put64(payload,sz); payload.insert(payload.end(),sha.begin(),sha.end()); ifstream f(o.input,ios::binary); vector<uint8_t>b(1<<20); while(f){f.read((char*)b.data(),b.size()); payload.insert(payload.end(),b.begin(),b.begin()+f.gcount());}
...
int decode_with_gpu(const Opt&o,const string&gpu){vector<uint8_t> bytes; for(auto&in:o.inputs){FILE*p=popen(ffmpeg_decode_cmd(in,gpu).c_str(),"r"); if(!p)die("failed to start ffmpeg"); vector<uint8_t> y(W*H); while(fread(y.data(),1,y.size(),p)==y.size()){uint8_t cur=0; int n=0; for(size_t bi=0;bi<BLOCKS;bi++){int bx=(bi%(W/BLOCK))*BLOCK, by=(bi/(W/BLOCK))*BLOCK; cur=(cur<<1)|(y[by*W+bx]>127); if(++n==8){bytes.push_back(cur); cur=0; n=0;}}} int rc=pclose(p); if(rc!=0)die("ffmpeg decode failed");}
 string ck=o.ckdecode&&!o.checkpoint.empty()?o.checkpoint:o.output+".decode.checkpoint"; if(!ends_with(ck,".decode.checkpoint")) die("decode checkpoint path must end with .decode.checkpoint"); decode_checkpoint(ck,o.inputs,o.output,0,"","started"); if(bytes.size()<49 || string(bytes.begin(),bytes.begin()+9)!="FSOYHARE1"){decode_checkpoint(ck,o.inputs,o.output,0,"","failed"); die("not an FSOY HARE v1 stream");} uint64_t sz=get64(bytes,9); array<uint8_t,32> sha{}; copy(bytes.begin()+17,bytes.begin()+49,sha.begin()); if(bytes.size()<49+sz){decode_checkpoint(ck,o.inputs,o.output,0,Sha256::hex(sha),"failed"); die("truncated stream");} ofstream out(o.output,ios::binary); out.write((char*)bytes.data()+49,sz); out.close(); auto got=file_sha(o.output); if(got!=sha){decode_checkpoint(ck,o.inputs,o.output,sz,Sha256::hex(sha),"failed"); die("SHA-256 verification failed: got "+Sha256::hex(got)+" expected "+Sha256::hex(sha));} decode_checkpoint(ck,o.inputs,o.output,sz,Sha256::hex(got),"complete"); cerr<<"Verified SHA-256 "<<Sha256::hex(got)<<"\n"; return 0;}

...
void die(const string&s){throw runtime_error(s);} bool have_ffmpeg(){return system("command -v ffmpeg >/dev/null 2>&1")==0;} uint64_t parse_size(string s){uint64_t m=1; char c=s.empty()?0:s.back(); if(!isdigit(c)){s.pop_back(); if(c=='K')m=1024; else if(c=='M')m=1024ull*1024; else if(c=='G')m=1024ull*1024*1024; else die("bad size suffix");} return stoull(s)*m;}
...
void checkpoint(const string&path,const string&src,const string&out,uint64_t size,const array<uint8_t,32>&sha,const string&status){ofstream c(path); c<<"{\n  \"source_path\": \""<<src<<"\",\n  \"source_size\": "<<size<<",\n  \"sha256\": \""<<Sha256::hex(sha)<<"\",\n  \"hare_version\": 1,\n  \"block_size\": 4,\n  \"ecc_ratio\": 0.10,\n  \"fps\": 24,\n  \"byte_offset\": "<<size<<",\n  \"volume_index\": 1,\n  \"outputs\": [\""<<out<<"\"],\n  \"status\": \""<<status<<"\"\n}\n";}
...
int encode(const Opt&o){
 ...
 if(o.gpu=="auto"||o.gpu=="frames"){
  ...
  if(gpu.loaded()&&gpu.available()){
   cerr<<"Loaded NVIDIA GPU Suite: "<<gpu.name()<<" ("<<gpu.status()<<")\n";
   frame_gpu=&gpu;
  } else {
   cerr<<"NVIDIA GPU Suite unavailable: "<<gpu.status()<<"; using CPU frame synthesis.\n";
  }
 }
...
string ffmpeg_decode_cmd(const string&i,const string&gpu){string accel; if(gpu=="auto"||gpu=="decode") accel="-hwaccel cuda "; return "ffmpeg -hide_banner -loglevel error "+accel+"-i '"+i+"' -f rawvideo -pix_fmt gray -";}
bool gpu_decode_requested(const string&gpu){return gpu=="auto"||gpu=="decode";}
bool decode_video_bytes(const string&in,const string&gpu,vector<uint8_t>&out){FILE*p=popen(ffmpeg_decode_cmd(in,gpu).c_str(),"r"); if(!p)die("failed to start ffmpeg"); vector<uint8_t> local; vector<uint8_t> y(W*H); while(fread(y.data(),1,y.size(),p)==y.size()){uint8_t cur=0; int n=0; for(size_t bi=0;bi<BLOCKS;bi++){int bx=(bi%(W/BLOCK))*BLOCK, by=(bi/(W/BLOCK))*BLOCK; cur=(cur<<1)|(y[by*W+bx]>127); if(++n==8){local.push_back(cur); cur=0; n=0;}}} int rc=pclose(p); if(rc!=0)return false; out.insert(out.end(),local.begin(),local.end()); return true;}
int decode(const Opt&o){if(!have_ffmpeg()) die("ffmpeg is required on PATH for MP4 encode/decode"); vector<uint8_t> bytes; for(auto&in:o.inputs){if(gpu_decode_requested(o.gpu)){cerr<<"GPU decode requested; FFmpeg CUDA/NVDEC will be attempted and CPU decode used if unavailable.\n"; if(decode_video_bytes(in,o.gpu,bytes)) continue; cerr<<"GPU decode failed; retrying with CPU FFmpeg decode.\n";} if(!decode_video_bytes(in,"off",bytes))die("ffmpeg decode failed");} if(bytes.size()<49 || string(bytes.begin(),bytes.begin()+9)!="FSOYHARE1") die("not an FSOY HARE v1 stream"); uint64_t sz=get64(bytes,9); array<uint8_t,32> sha{}; copy(bytes.begin()+17,bytes.begin()+49,sha.begin()); if(bytes.size()<49+sz)die("truncated stream"); ofstream out(o.output,ios::binary); out.write((char*)bytes.data()+49,sz); out.close(); auto got=file_sha(o.output); if(got!=sha)die("SHA-256 verification failed: got "+Sha256::hex(got)+" expected "+Sha256::hex(sha)); cerr<<"Verified SHA-256 "<<Sha256::hex(got)<<"\n"; return 0;}
Opt parse(int argc,char**argv){ if(argc<2)die("usage: fsoy encode <file> -o <out.mp4> | fsoy decode <mp4...> -o <file>"); Opt o; o.mode=argv[1]; for(int i=2;i<argc;i++){string a=argv[i]; if(a=="-o"&&i+1<argc)o.output=argv[++i]; else if(a=="--gpu"&&i+1<argc)o.gpu=argv[++i]; else if(a=="--gpu-plugin"&&i+1<argc)o.gpu_plugin=argv[++i]; else if(a=="--max-output-size"&&i+1<argc)o.max_size=parse_size(argv[++i]); else if(a=="--auto-continue")o.auto_continue=true; else if(a=="--resume"&&i+1<argc){o.resume=true; o.checkpoint=argv[++i];} else if(a=="--checkpoint"&&i+1<argc){o.ckdecode=true; o.checkpoint=argv[++i];} else if(o.mode=="decode")o.inputs.push_back(a); else if(o.input.empty())o.input=a; else die("unexpected arg "+a);} if(o.mode=="encode"&&!o.resume&&(o.input.empty()||o.output.empty()))die("encode needs input and -o"); if(o.mode=="decode"&&(o.inputs.empty()||o.output.empty()))die("decode needs input video(s) and -o"); return o;}

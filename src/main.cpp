...
string ffmpeg_encode_cmd(const string&o,const string&gpu){ string enc="libx264 -preset veryfast -crf 0 -pix_fmt yuv420p"; if(gpu=="auto"||gpu=="encode") enc="h264_nvenc -preset lossless -rc constqp -qp 0 -pix_fmt yuv420p"; return "ffmpeg -hide_banner -loglevel error -y -f yuv4mpegpipe -i - -c:v "+enc+" -movflags +faststart+frag_keyframe+empty_moov "+"'"+o+"'"; }
